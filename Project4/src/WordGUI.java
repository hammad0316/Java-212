...
public class WordGUI extends JFrame {	
	...
	public static void addText(SortedWordList sorted, UnsortedWordList unsorted) {
		...
		TreeMap<Word, Integer> wordTreeMap = sorted.getwordTreeMap();
		
		//iterator which will loop through the tree map
		Set set = wordTreeMap.entrySet(); 
		Iterator i = set.iterator(); 

		while(i.hasNext()) { 
			Map.Entry me = (Map.Entry)i.next(); 
			int loops = (int) me.getValue();
			for(int k = 0; k<loops; k++){
				textArea1.append(((Word)me.getKey()).word + "\n");				
			}
		} 
